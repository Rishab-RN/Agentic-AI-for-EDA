...
class EdgeCaseConfig:
    ...
    groq_model: str = "compound-beta"  # Groq Compound
    groq_timeout: int = 30
...
class GroqLLMResolver:
    """Resolves semantic edge cases using Groq Compound LLM."""
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("EDGE CASE HANDLER TEST")
    print("=" * 60)
    
    # Create test DataFrame with various edge cases
    test_df = pd.DataFrame({
        "id": [1, 2, 3, 4, 5],
        "customer_id": [101, 102, 103, 104, 105],
        "created_at": ["2024-01-01", "2024-01-02", "2024-01-03", "2024-01-04", "2024-01-05"],
        "price": [10.5, 20.3, 15.7, np.inf, 25.0],  # Has inf
        "constant_col": [1, 1, 1, 1, 1],  # Single value
        "name/with/slashes": ["a", "b", "c", "d", "e"],  # Special chars
        "empty_col": [None, None, None, None, None],  # 100% missing
        "binary_flag": [0, 1, 0, 1, 0],  # Binary numeric
        "rating": [1, 2, 3, 4, 5],  # Ambiguous type
    ...
    for col in test_df.columns:
        meta = {
            ...
            "is_categorical": pd.api.types.is_object_dtype(test_df[col]),
            "missing_pct": test_df[col].isna().mean() * 100,
            "valid_count": test_df[col].notna().sum(),
            "cardinality": test_df[col].nunique()
        }
        if meta["is_numeric"]:
            meta["variance"] = test_df[col].var() if not test_df[col].isna().all() else 0
        test_metadata[col] = meta
    
    # Run detection
    detector = EdgeCaseDetector()
    edge_cases = detector.detect_all(test_df, test_metadata)
    
    print(f"\nDetected {len(edge_cases)} edge cases:")
    for ec in edge_cases:
        print(f"  - {ec.case_type.value}: {ec.columns}")
        print(f"    Requires LLM: {ec.requires_llm}")
        print(f"    Details: {ec.details}")
    
    # Run full processing (if Groq available)
    print("\n" + "-" * 60)
    print("Running full edge case processing...")
    result = process_edge_cases(test_df, test_metadata)
    
    print(f"\nProcessing Results:")
    print(f"  Should abort: {result['should_abort']}")
    print(f"  Columns to skip: {result['columns_to_skip']}")
    print(f"  Type overrides: {result['column_type_overrides']}")
    print(f"  Filename sanitization: {result['filename_sanitization']}")
