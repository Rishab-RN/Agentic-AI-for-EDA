...
def run_pipeline_simple(input_csv_path: str, output_dir: str = ".") -> dict:
    ...
    serializable_report = {
        ...
        "edge_case_handling": {
            "columns_skipped": state.get("columns_to_skip", []),
            "type_overrides": state.get("column_type_overrides", {}),
...
def print_summary(state: dict):
    ...
    # Edge case handling summary (new)
    columns_to_skip = state.get("columns_to_skip", [])
    target_columns = state.get("target_columns", [])
    datetime_columns = state.get("datetime_columns", [])
    
    if columns_to_skip or target_columns or datetime_columns:
        print(f"\n[EDGE CASES] LLM-Enhanced Analysis:")
        if columns_to_skip:
            print(f"   Columns skipped: {len(columns_to_skip)}")
            for col in columns_to_skip[:5]:
                print(f"      - {col}")
            if len(columns_to_skip) > 5:
                print(f"      ... and {len(columns_to_skip) - 5} more")
        if target_columns:
            print(f"   Target columns detected: {target_columns}")
        if datetime_columns:
...
if __name__ == "__main__":
    # Default test file
    default_file = r"C:\Users\rohan\Antigravity\EL_sem3\visualization_agent\Titanic-Dataset_cleaned_cleaned_20260118_161450.csv"
